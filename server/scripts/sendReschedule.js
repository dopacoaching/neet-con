/**
 * One-off: send the "event rescheduled to evening 7:30 PM today" update to
 * every confirmed CareerX registrant (event=careerx, seat-holding status —
 * i.e. has a registrationNumber), via the approved
 * WHATSAPP_RESCHEDULE_TEMPLATE_NAME template (careerx_reschedule_evening_v1).
 * Tracked per-registration in rescheduleSentAt so this script is safe to
 * re-run (only unsent ones go out).
 *
 * Usage:
 *   cd server
 *   node scripts/sendReschedule.js --dry-run          # preview recipients, sends nothing
 *   node scripts/sendReschedule.js --test=9876543210  # send ONE real message to this number only (sample data, not tracked in DB)
 *   node scripts/sendReschedule.js                     # actually sends to every unsent registrant
 */
...
const EVENT = {
  ...
  time: process.env.EVENT_TIME || '7:30 PM',
};
...
async function sendRescheduleTo(reg) {
  const payload = {
...
const run = async () => {
  ...
  if (testNumber) {
    console.log(`[reschedule] TEST MODE — sending one real message to ${testNumber} (sample data, DB untouched).`);
    const result = await sendRescheduleTo({
      fullName: 'Test User',
      registrationNumber: 'TEST 000',
      mobileNumber: testNumber,
    });
    console.log(result.sent ? `[reschedule] Test message sent.` : `[reschedule] Test send FAILED: ${result.reason}`);
  ...
  const targets = await Registration.find({
    ...
    rescheduleSentAt: null,
  }).sort({ createdAt: 1 });

  console.log(`[reschedule] ${targets.length} confirmed CareerX registrant(s) to notify.`);

  if (dryRun) {
    targets.forEach((r) =>
      console.log(`  - ${r.registrationNumber}  ${r.fullName}  ${r.mobileNumber}`)
    );
    console.log(`\n[reschedule] Dry run — nothing sent.`);
  ...
  for (const reg of targets) {
    const result = await sendRescheduleTo(reg);
    if (result.sent) {
      await Registration.updateOne({ _id: reg._id }, { $set: { rescheduleSentAt: new Date() } });
      sent += 1;
    } else {
      console.warn(`[reschedule] failed ${reg.fullName} ${reg.mobileNumber}: ${result.reason}`);
      failed += 1;
    }
    await sleep(DELAY_MS);
  }

  console.log(`\n[reschedule] Done. Sent: ${sent}, failed: ${failed}.`);
  await mongoose.connection.close();
  process.exit(0);
};
